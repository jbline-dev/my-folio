...
export function CommandBar() {
  ...
  useEffect(() => {
    ...
    if (!isDeleting && charIdx < currentMsg.length) {
      ...
      timeout = setTimeout(() => setCharIdx((prev: number) => prev + 1), speed);
      ...
      // Fast delete
      timeout = setTimeout(() => setCharIdx((prev: number) => prev - 1), 15);
    } else if (isDeleting && charIdx === 0) {
      // Move to next message
      setIsDeleting(false);
      setMsgIdx((prev: number) => (prev + 1) % MESSAGES.length);
    }
  ...
  return (
    ...
    >
      <div className="flex min-w-0 flex-1 items-center gap-2">
        <ChevronRight className="size-4 shrink-0 text-[var(--pf-muted)]" />
        <div className="flex h-4 items-center font-mono text-[10px] text-[var(--pf-fg)] sm:text-xs">
          {text}
          <span className="ml-1 inline-block h-3 w-1.5 animate-pulse bg-[var(--pf-muted)] sm:h-3.5" />
        </div>
      </div>
      ...
      <div className="flex shrink-0 items-center gap-4">
        <span
          className={`hidden font-mono text-[9px] uppercase tracking-widest transition-colors sm:inline ${isPaused ? "text-[var(--pf-fg)]" : "text-[var(--pf-faint)]"}`}
        >
          [ Hover / Hold to pause ]
        </span>
        <span
          className={`inline font-mono text-[9px] uppercase tracking-widest transition-colors sm:hidden ${isPaused ? "text-[var(--pf-fg)]" : "text-[var(--pf-faint)]"}`}
        >
          [ Hold to pause ]
        </span>
